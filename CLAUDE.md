...
- `torch`/`safetensors` are **direct production dependencies**, not transitive ones: `mlx-transformers` 0.3.0 dropped `torch` and its loader now rejects `.bin` checkpoints outright (`_discover_safetensor_files` raises "supports safetensors only"). Since `siebert/sentiment-roberta-large-english` ships only `pytorch_model.bin` on `main`, `_ensure_safetensors` must do the conversion itself, so both packages are declared where they are imported. Verify with `uv tree`, never by assuming a transitive path still holds
...
- As of ruff 0.16, `ruff format` also formats Python code fences inside Markdown, so it covers 6 files here, not 4 — `CLAUDE.md` and `README.md` are now gated by CI's `ruff format --check`. Currently a no-op (every fence is ```bash or ```bibtex), but a ```python snippet added to either file must be ruff-formatted or the docs-only change fails CI
